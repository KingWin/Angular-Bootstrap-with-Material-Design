...
export async function compilePackagesWithNgc(config: Config) {
  ...
  const restPkgs = pkgs.filter(name => name !== utilsPkg);

  await _compilePackagesWithNgc(utilsPkg);
...
export async function cleanJavaScriptFiles(config: Config) {
  ...
  const jsExcludeFilesFlob = [
    './dist/packages/(bundles|angular-bootstrap-md)/**/*.js',
    './dist/**/((?-)testing)/**/*.js'
  ];
  ...
  const filteredFilesToRemove = filesToRemove.filter((file: string) => {
    return packages.length ? packages.some(pkg => file.indexOf(pkg) === -1) : true;
  });
...
export function mapAsync<T>(list: T[], mapFn: (v: T, i: number) => Promise<any>) {
 ...
 */
export async function copyAssetsFiles(config: Config) {
  const fontFiles = await util.getListOfFiles(`./modules/font/*`);
  const imgFiles = await util.getListOfFiles(`./modules/img/*`);
  const scssFiles = await util.getListOfFiles(`./modules/scss/*`);
  const files = [...fontFiles, ...scssFiles, ...imgFiles];
